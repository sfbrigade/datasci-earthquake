...
# def check_hazard_exists(
#     model: str, geom_column_name: str, lon: float, lat: float, db: dict
# ):
def check_hazard_exists(
    model: Type[Any], geom_column_name: str, lon: float, lat: float, db: Session
):
    """
...
def lookup_all_hazards(
    ...
    try:
        ...
        logging.info(
            f"Soft story check complete: exists={soft_story_exists}, updated={soft_story_updated}"
        )
        ...
        logging.info(
            f"Liquefaction check complete: exists={liquefaction_exists}, updated={liquefaction_updated}"
        )
        ...
        logging.info(
            f"Tsunami check complete: exists={tsunami_exists}, updated={tsunami_updated}"
        )

        response = CompositeHazardResponse(
            soft_story=HazardStatus(
                exists=soft_story_exists, last_updated=soft_story_updated
            ),
            liquefaction=HazardStatus(
                exists=liquefaction_exists, last_updated=liquefaction_updated
            ),
