...
def prepare(input_path: str, output_path: str) -> None:
    ...
    # Deliberately not simplifying geometry: per-tract simplify() only
    # preserves topology within a single polygon, not shared boundaries
    # between adjacent tracts, so it can open gaps/overlaps at tract edges
    # and make ST_Intersects miss or mis-assign points near a boundary -
    # unacceptable for a point-lookup API. The dataset is still small
    # (241 SF tracts, ~770KB) without simplification.
