...
def test_delete_tsunami_polygon(client):
    response = client.delete("/api/polygons/1?database_name=tsunami")
...
def test_put_tsunami_polygon(client):
    response = client.put("/api/polygons/1?database_name=tsunami",
...
def test_get_tsunami_polygon(client):
    response = client.get("/api/polygons/1?database_name=tsunami")
    assert response.status_code == 200
    # Temporary guaranteed failure until test is written
    assert False
